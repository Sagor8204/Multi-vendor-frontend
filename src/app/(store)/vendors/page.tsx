      ...
      <div className="bg-white border-b border-border/60 mb-16 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 py-24 relative z-10">
          ...
          <p className="text-xl text-muted font-medium max-w-2xl leading-relaxed">
            Support independent creators and global brands. Every vendor on our
            platform is hand-vetted for quality and reliability.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {[
            ...
            <div
              key={i}
              className="bg-white p-6 rounded-3xl border border-border/40 shadow-sm flex items-center gap-4"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            [1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="h-[400px] rounded-[2.5rem] bg-white animate-pulse border border-border/40"
              />
            ))
          ) : vendors.length > 0 ? (
            vendors.map((vendor) => (
              <Card
                key={vendor.id}
                noPadding
                className="group bg-white rounded-[2.5rem] border-transparent hover:border-secondary/20 hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] transition-all duration-500 overflow-hidden flex flex-col h-full"
              >
                ...
                <div className="h-32 bg-background-subtle relative overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4">
                    {vendor.is_verified && (
                      <Badge
                        variant="secondary"
                        className="bg-white/90 backdrop-blur-md shadow-sm border-none"
                      >
                        Verified Store
                      </Badge>
                    )}
                ...
                <div className="px-8 pb-10 flex-grow relative pt-12">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-end">
                      ...
                      <div className="text-right">
                        <span className="text-xs font-black text-muted uppercase tracking-widest block">
                    ...
                    <p className="text-sm text-muted font-medium line-clamp-3 leading-relaxed">
                      {vendor.store_description ||
                        "This store hasn't provided a description yet. Explore their unique collection of products below."}
                    </p>
                  </div>
                  ...
                  <div className="mt-8 flex items-center gap-4">
                    <Link href={`/vendors/${vendor.id}`} className="flex-grow">
                      <Button
                        variant="secondary"
                        className="w-full h-12 text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-secondary/20"
                      >
                        Visit Store
                      </Button>
                    </Link>
            ...
            <div className="col-span-full py-24 text-center bg-white rounded-[3rem] border border-border/60 shadow-sm">
              ...
              <h3 className="text-2xl font-black text-main tracking-tight italic">
                No vendors found
              </h3>
              <p className="text-muted font-semibold mt-2">
                Try checking back later for new stores.
              </p>
            </div>
          )}
        </div>
                ...
                {isAuthenticated && user?.role === "vendor" ? (
                  <>
                    ...
                    <p className="text-white/60 text-lg font-medium leading-relaxed">
                      Your dashboard is waiting. Update products, check sales,
                  ...
                  <>
                    <h2 className="text-4xl md:text-6xl font-black text-white tracking-tighter leading-none mb-6">
                      Ready to grow{" "}
                      <span className="text-secondary italic">bigger?</span>
                    </h2>
                    <p className="text-white/60 text-lg font-medium leading-relaxed">
                      Join 50,000+ creators who sell their products on
                      MarketPro. Reach millions of customers with professional
                      tools.
                    </p>
                  </>
                )}
              ...
              <div>
                {isAuthenticated && user?.role === "vendor" ? (
                  <Link href="/vendor">
                    <Button
                      variant="secondary"
                      className="px-12 py-5 h-auto text-sm font-black uppercase tracking-widest shadow-2xl shadow-secondary/40 flex items-center gap-3"
                    >
