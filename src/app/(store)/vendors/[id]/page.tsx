    ...
    return (
      <div className="max-w-7xl mx-auto px-6 py-20 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           {[1,2,3].map(i => <div key={i} className="h-80 bg-background-subtle rounded-3xl" />)}
        </div>
      </div>
    );
  }
  ...
  if (!vendor) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-32 text-center">
         ...
         <p className="text-muted mt-2">The vendor you are looking for might have closed or moved.</p>
           ...
           <div className="flex flex-col md:flex-row gap-8 items-end -translate-y-1/2 md:-translate-y-12">
              ...
              <div className="flex-grow pb-2 md:pb-0">
                 <div className="flex flex-wrap items-center gap-3 mb-3">
                    ...
                    {vendor.is_verified && (
                      <Badge variant="secondary" className="px-3 py-1">Verified Partner</Badge>
                    )}
                 </div>
                 ...
                 <div className="flex flex-wrap items-center gap-6 text-sm font-semibold text-muted">
                    <div className="flex items-center gap-1.5 text-warning">
                       ...
                       <span className="text-main font-black">{vendor.rating || 'New'}</span>
                       <span className="text-[10px] uppercase opacity-60">(Reviews coming soon)</span>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
           ...
           <aside className="lg:col-span-4 space-y-10">
              <div className="space-y-6">
                 <h3 className="text-[10px] font-black text-main uppercase tracking-[0.2em]">About the Store</h3>
                 <p className="text-base text-muted font-medium leading-relaxed">
                    {vendor.store_description || "Welcome to our premium store. We are dedicated to providing the highest quality products and exceptional customer service to our global community."}
                 </p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                 <Card noPadding className="p-6 bg-white border-border/40 shadow-sm">
                    <div className="text-2xl font-black text-main tracking-tighter">{vendor.product_count || products.length}</div>
                    <div className="text-[10px] font-bold text-muted uppercase tracking-widest mt-1">Total Products</div>
                 </Card>
                 <Card noPadding className="p-6 bg-white border-border/40 shadow-sm">
                    <div className="text-2xl font-black text-secondary tracking-tighter">100%</div>
                    <div className="text-[10px] font-bold text-muted uppercase tracking-widest mt-1">Response Rate</div>
                 </Card>
              </div>
              ...
              <div className="p-8 bg-main rounded-[2rem] text-white relative overflow-hidden group">
                 ...
                 <h4 className="text-xl font-black mb-2 relative z-10">MarketPro Guarantee</h4>
                 <p className="text-white/60 text-sm font-medium leading-relaxed relative z-10">
                    Every purchase from this verified vendor is protected by our global safety program.
                 </p>
           ...
           <div className="lg:col-span-8">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center mb-10 pb-6 border-b border-border/60 gap-6">
                <div>
                   <h2 className="text-xl font-black text-main tracking-tight italic">Store Collection</h2>
                   <p className="text-xs font-semibold text-muted mt-1">Showing {products.length} exclusive items</p>
                </div>
                ...
                <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                   ...
                   <div className="flex items-center gap-1 p-1 bg-white rounded-xl border border-border/60 shadow-sm">
                      <button 
                        ...
                        className={`p-1.5 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-secondary text-white shadow-md' : 'text-muted hover:text-main'}`}
                      >
                        <LayoutGrid className="w-4 h-4" />
                      </button>
                      <button 
                        onClick={() => setViewMode('list')}
                        className={`p-1.5 rounded-lg transition-all ${viewMode === 'list' ? 'bg-secondary text-white shadow-md' : 'text-muted hover:text-main'}`}
                      >
              ...
              {productsLoading ? (
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    {[1, 2, 3, 4].map(i => (
                      <div key={i} className="aspect-[4/5] bg-white animate-pulse rounded-3xl border border-border/40" />
                    ))}
                 </div>
              ) : products.length > 0 ? (
                ...
                <div className="py-24 text-center bg-white rounded-[3rem] border border-border/60 shadow-sm">
                   ...
                   <h3 className="text-2xl font-black text-main tracking-tight">No products found</h3>
                   <p className="text-muted font-semibold mt-2">This store hasn't listed any products yet.</p>
                </div>
              )}
           </div>
        </div>
