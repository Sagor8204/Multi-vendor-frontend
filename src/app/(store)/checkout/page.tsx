...
function CheckoutContent() {
  ...
  const { addresses, isLoadingAddresses, addAddress, isAddingAddress } =
    useUser();
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-32 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-muted max-w-md mb-10 font-bold uppercase tracking-widest text-[10px]">
            ...
            {isLoadingAddresses ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {addresses.map((addr) => (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4">
                      <div
                        className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-colors ${selectedAddressId === addr.id ? "bg-primary/10 text-primary" : "bg-slate-100 text-muted"}`}
                      >
                        <MapPin className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="font-black text-main text-sm uppercase tracking-tight mb-2">
                          {addr.full_name}
                        </div>
                ...
                >
                  ...
                  <span className="text-[10px] font-black text-muted group-hover:text-primary uppercase tracking-[0.2em]">
                    Add New Destination
                  </span>
                </div>
              </div>
          ...
          >
            <div className="border-2 border-primary/20 bg-white p-8 rounded-4xl flex items-center justify-between shadow-sm">
              <div className="flex items-center space-x-5">
                ...
                <div>
                  <div className="font-black text-main uppercase tracking-tight text-base mb-1">
                    Credit or Debit Card
                  </div>
                  <div className="text-[10px] text-muted font-black uppercase tracking-[0.2em]">
                    Encrypted & Secure SSL Checkout
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-32 space-y-8">
            <Card className="bg-text-main text-white border-none shadow-2xl relative overflow-hidden p-4 rounded-[48px]">
              ...
              <h2 className="text-2xl font-black mb-10 text-secondary tracking-tighter relative z-10">
                Review Order.
              </h2>
              ...
              <div className="space-y-6 max-h-80 overflow-y-auto pr-4 custom-scrollbar relative z-10">
                {cart.map((item) => (
                  <div
                    key={item.id}
                    className="flex justify-between items-center gap-6 group"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-sm font-black line-clamp-1 text-muted group-hover:text-secondary transition-colors mb-1">
                          {item.name}
                        </div>
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="flex justify-between items-center pt-6 border-t border-white/5 mt-6">
                  <span className="text-xs font-black text-muted uppercase tracking-[0.3em]">
                    Total Amount
                  </span>
